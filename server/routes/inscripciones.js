...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (duplicado.rows.length > 0) {
      return res.status(400).json({ error: 'El estudiante ya está inscripto en este curso' });
    }
    ...
    if (curso.rows[0].estado !== 'activo') {
      return res.status(400).json({ error: 'Las inscripciones no están habilitadas para este curso' });
    }
    ...
    if (inscriptosMax && cantidadActual >= inscriptosMax) {
      return res.status(400).json({ error: 'No hay cupo disponible en este curso' });
    }
...
router.delete('/:id', async (req, res) => {
  try {
    const result = await pool.query('DELETE FROM inscripciones WHERE id = $1 RETURNING *', [req.params.id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Inscripción no encontrada' });
    }
